...
$aeocas_pending_studio_connect_token = AEOCAS_Settings::get_requested_studio_connect_token();
					...
					<?php if ( $aeocas_connected ) : ?>
						...
						<?php else : ?>
							<div class="aeo-connect-card aeo-connect-card-disconnected">
								...
								<?php if ( $aeocas_can_manage ) : ?>
									<?php if ( '' !== $aeocas_pending_studio_connect_token ) : ?>
										<div class="notice notice-warning" style="margin: 0 0 16px;">
											<p><strong><?php esc_html_e( 'Studio requested a secure connection for this site.', 'aeo-content-ai-studio' ); ?></strong></p>
											<p><?php esc_html_e( 'Review the request and confirm it here in WordPress admin to finish connecting this site.', 'aeo-content-ai-studio' ); ?></p>
											<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="aeo-inline-form">
												<input type="hidden" name="action" value="<?php echo esc_attr( AEOCAS_Settings::STUDIO_CONNECT_ACTION ); ?>" />
												<input type="hidden" name="connect_token" value="<?php echo esc_attr( $aeocas_pending_studio_connect_token ); ?>" />
												<?php wp_nonce_field( AEOCAS_Settings::STUDIO_CONNECT_ACTION ); ?>
												<button type="submit" class="button button-primary"><?php esc_html_e( 'Complete Secure Connection', 'aeo-content-ai-studio' ); ?></button>
											</form>
										</div>
